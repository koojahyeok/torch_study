...
class NerfModel(nn.Module):
    def __init__(self, embedding_dim_pos=10, embedding_dim_direction=4, hidden_dim=128):  ## 논문 하이퍼 파라미터 세팅
        ...
        def compute_accumulated_transmittance(alphas):
            ...
            return torch.cat((torch.ones((accumulated_transmittance[0], 1), device=alphas.device),
        ...
        def render_rays(nerf_model, ray_origins, ray_directions, hn=0, hf=0.5, nb_bins=192):
            ...
            color, sigma = nerf_model(x.reshape(-1, 3), ray_directions.reshape(-1, 3))
